...
valueParser ::
  a
  -> Parser a
valueParser s = P (\input -> Result input  s)
...
bindParser a3pb pa = 
  P (\input -> case (parse pa) input of
                  ...
                  Result input2 a -> parse (a3pb a) input2 
                  )
...
list ::
  Parser a
  -> Parser (List a)
list parser =  list1 parser ||| pure Nil
  -- P(\input -> case parse input parser of )
...
--           pure (a:.as)
  ...
  -- (1 or many) P = p and then call it a 
...
-- parse character and then call it c
-- if this predicate c
  -- then always C
  -- else unexepectedCharParser c
...
-- list = list >>= pure (\x ->
--   ) 
    -- get this to pass without sequence
...
personParser = 
  do
    ...
    _ <- spaces1
    phone <- phoneParser
    pure (Person age fname lname smoker phone)
